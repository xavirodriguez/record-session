...
export async function deleteSessionAction(sessionId, actionId) {
  ...
  const actionsKey = `${ACTIONS_PREFIX}${sessionId}`;
  await chrome.storage.local.set({ [actionsKey]: filteredActions });

  // Actualizar contador en metadata
  const metadataList = await getSessionsMetadata();
  const index = metadataList.findIndex(m => m.id === sessionId);
  if (index !== -1) {
    metadataList[index].actionCount = filteredActions.length;
    await chrome.storage.local.set({ [METADATA_KEY]: metadataList });
  }
}
...
export async function reorderSessionActions(sessionId, actions) {
    ...
    await chrome.storage.local.set({ [actionsKey]: validatedActions });
}

export async function clearAllSessions() {
    const metadata = await getSessionsMetadata();
    const actionKeys = metadata.map(s => `${ACTIONS_PREFIX}${s.id}`);

    // Eliminar todos los shards de acciones y luego los metadatos
    await chrome.storage.local.remove([...actionKeys, METADATA_KEY]);
}
