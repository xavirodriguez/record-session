...
/**
 * Elimina una acción de un shard de sesión y actualiza el contador
 */
export async function deleteSessionAction(sessionId, actionId) {
  const actionsKey = `${ACTIONS_PREFIX}${sessionId}`;
  const res = await chrome.storage.local.get([actionsKey, METADATA_KEY]);

  let actions = res[actionsKey] || [];
  const metadataList = res[METADATA_KEY] || [];

  const initialCount = actions.length;
  actions = actions.filter(a => a.id !== actionId);

  if (actions.length < initialCount) {
    const metaIndex = metadataList.findIndex(m => m.id === sessionId);
    if (metaIndex !== -1) {
      metadataList[metaIndex].actionCount = actions.length;
    }
...
/**
 * Reordena las acciones en un shard de sesión
 */
export async function reorderSessionActions(sessionId, actions) {
  const actionsKey = `${ACTIONS_PREFIX}${sessionId}`;
  // Validar que las acciones entrantes son válidas
