...
/**
 * Performance Optimization: Batch Screenshot Retrieval
 *
 * Retrieves multiple screenshots in a single IndexedDB transaction to reduce overhead.
 * This is significantly more efficient than multiple individual `getScreenshot` calls.
 *
 * @param {string[]} ids - An array of screenshot or element IDs to retrieve.
 * @returns {Promise<{[id: string]: string | null}>} - A map of IDs to data URLs.
 */
export async function getScreenshotsBatch(ids) {
  ...
  await Promise.all(ids.map(async (id) => {
    try {
      let item = await promisifyRequest(elementsStore.get(id));
      if (!item) {
        item = await promisifyRequest(store.get(id));
      }
      ...
      console.error(`Failed to retrieve screenshot for ID ${id}:`, error);
      results[id] = null;
    }
  }));
