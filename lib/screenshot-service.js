...
export async function getScreenshotsBatch(ids) {
  ...
  const db = await openDatabase();
  const tx = db.transaction([STORE_NAME, EXTRACTED_ELEMENTS_STORE], 'readonly');
  const screenshotsStore = tx.objectStore(STORE_NAME);
  const elementsStore = tx.objectStore(EXTRACTED_ELEMENTS_STORE);
  const results = {};

  const promises = ids.map(id => {
    return new Promise((resolve, reject) => {
      // Priorizar la obtención desde 'extractedElements'
      const req = elementsStore.get(id);
      req.onsuccess = () => {
        if (req.result) {
          resolve({ id, item: req.result });
        } else {
          // Si no se encuentra, buscar en 'screenshots'
          const scrReq = screenshotsStore.get(id);
          scrReq.onsuccess = () => resolve({ id, item: scrReq.result });
          scrReq.onerror = () => reject(scrReq.error);
        }
      };
