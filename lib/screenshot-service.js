...
export async function getScreenshotsBatch(ids) {
  ...
  const elementsStore = tx.objectStore(EXTRACTED_ELEMENTS_STORE);
  const results = {};

  // Helper to process a single get request and convert blob to data URL
  const fetchAndConvert = (store, id) => {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (item && item.data instanceof Blob) {
          const reader = new FileReader();
          reader.onloadend = () => resolve({ id, dataUrl: reader.result });
          reader.onerror = () => resolve({ id, dataUrl: null }); // Resolve null on error
          reader.readAsDataURL(item.data);
        } else {
          resolve({ id, dataUrl: null }); // Resolve null if not found or no blob
        }
      };
      // Important: Handle a failed get request to prevent Promise.all from hanging
      request.onerror = () => resolve({ id, dataUrl: null });
    });
  };

  // Create a list of promises for all IDs
  const promises = ids.map(id => {
    // Decide which store to query based on ID prefix
    const store = id.startsWith('el_') ? elementsStore : objectStore;
    return fetchAndConvert(store, id);
  });
  ...
  for (const result of settledResults) {
    if (result.dataUrl) {
      results[result.id] = result.dataUrl;
    }
  }
