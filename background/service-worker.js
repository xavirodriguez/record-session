...
async function handleAction(action, tab) {
  ...
  if (['click', 'input', 'submit'].includes(action.type) && tab?.id) {
    try {
      ...
      const QUALITY_MAP = { low: 30, medium: 60, high: 90 };
      const qualityValue = QUALITY_MAP[config.quality] || 60;

      const dataUrl = await new Promise((resolve, reject) => {
        chrome.tabs.captureVisibleTab(null, { format: 'jpeg', quality: qualityValue }, (dataUrl) => {
